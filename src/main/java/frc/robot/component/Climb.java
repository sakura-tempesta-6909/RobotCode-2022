...
public class Climb implements Component {
   ...
   * 
   * {うまくかかっていないときは}
   *  ロボットがミッドラング側に少し上がるまで左バンパーを押す
   *  >>>ロボットがミッドラング側に少し上がる
   * 
   *  左トリガーの入力を少し入れながら右バンパーを押しファーストを外す
   *  >>>ファーストの爪が少し上がる
   * 
   *  右バンパーを押していたのを離してファーストをとじる
   * {ここでしっかりかかるはず}
   * 
   * 左のバンパーを押すとともに右のトリガーを半押しする
   * セカンドが外れるとともに右のトリガーをゆっくり緩めていく
   * ちょい押しまで緩める
   * >>>セカンドが外れ、ロボットが振り子のような動きをする。振り子の動きが小さいほど良し
   * 
   * 左トリガーをゆっくり半押しまで押す
   * >>>セカンドがトラヴァーサルラングに引っ掛かる
   * 
   * {うまくかかっていないときは}
   *  ロボットがミッドラング側に少し上がるまで左トリガーを押す
   *  >>>ロボットがミッドラング側に少し上がる
   * 
   *  左トリガーの入力を少し入れながら左バンパーを押しセカンドを外す
   *  >>>ファーストの爪が少し上がる
   * 
   *  左バンパーバンパーを押していたのを離してセカンドをとじる
   * {ここでしっかりかかるはず}
   * ファーストを外すとともに右のトリガー半押し
   * ファーストが外れたら右のトリガーをちょい押しまで緩める
   * ロボットのアームが縦になるまで回す
   * 
   * <<<<<クライム完了>>>>>
   */
